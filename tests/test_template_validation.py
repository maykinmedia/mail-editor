...
class TemplateValidationTests(TestCase):
    def test_valid_template(self):
        template = MailTemplate(template_type="template", subject="{{ foo }}", body="{{ bar }}")
        try:
    ...
    def test_template_syntax_error(self):
        template = MailTemplate(template_type="template", subject="{{ foo bar }}", body="{{ bar }}")
    ...
    def test_template_invalid_error(self):
        template = MailTemplate(template_type="template", subject="{{ bar }}", body="{{ bar }}")
    ...
    def test_valid_template_with_attribute(self):
        template = MailTemplate(template_type="template", subject="{{ foo.bar }}", body="{{ bar.foo }}")
        try:
            validate_template(template)
        except ValidationError:
            self.fail("Unexpected validationError")
    ...
    def test_valid_template_with_unknown_variable(self):
        template = MailTemplate(template_type="template", subject="{{ foobar }}", body="{{ bar }}")
